...
public class WorklogBean {
	...
	public boolean list(HttpServletRequest request, String username,
		...
		if (db.createConn()) {
			...
			// 取得当前页数据SQL
			String sql2 = "select * from worklog where username='" + username
	...
	public boolean select(HttpServletRequest request, String username) {
		...
		if (db.createConn()) {
			...
			if (db.next()) {
				...
				request.setAttribute("description", db.getValue("description"));
				request.setAttribute("logtime", db.getValue("logtime"));
			}
			db.closeRs();
			db.closeStm();
			db.closeConn();
