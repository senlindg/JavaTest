...
public class MeetingBean {
	...
	public boolean list(HttpServletRequest request, String username, String strPageSize,
		...
		if(db.createConn()) {
			...
			// 取得当前页数据SQL
			String sql2 = "select * from meeting order by starttime desc limit "
				+ start + "," + pageSize;
			...
			while (db.next()) {
				...
				hash.put("starttime", db.getValue("starttime"));
				hash.put("endtime", db.getValue("endtime"));
	...
	public boolean insert(HttpServletRequest request, String username) {
		...
		if(db.createConn()) {
			...
			sql += " values('" + username + "','" + starttime + "','" + endtime
	...
	public boolean select(HttpServletRequest request, String username) {
		...
		if(db.createConn()) {
			...
			if (db.next()) {
				...
				request.setAttribute("starttime", db.getValue("starttime"));
				request.setAttribute("endtime", db.getValue("endtime"));
	...
	public boolean update(HttpServletRequest request, String username) {
		...
		if(db.createConn()) {
			...
			// 组合更新SQL
			String sql = "update meeting set starttime='" + starttime
